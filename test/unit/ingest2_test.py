from hsman.ingest2 import _get_common_idx, _make_dataset_folder, \
...
def test_make_dataset_folder(tmp_path):
    _make_dataset_folder(os.path.join(tmp_path, 'TEST1'))
...
def test_ingest_hsi(tmp_path):
    ds1 = generate_rotated_raster(tmp_path, True)
    ds2 = generate_rotated_raster(tmp_path, False)
    ingest_hsi_sequence([ds1, ds2], 'TEST01', tmp_path)
